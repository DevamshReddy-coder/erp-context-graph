...
export async function POST(req: Request) {
  try {
    ...
    // --- ANALYTICAL HARD-PLEDGE (V5.0 PRODUCTION) ---
    if ((lowerQuery.includes("trace") || lowerQuery.includes("journey")) && /\d+/.test(lowerQuery)) {
        ...
        offlineOverrideText = `🛡️ **V5.0 ANOMALY DETECTOR**: Analyzed global context for O2C flow leakage.\n\nI have identified **high-risk Sales Orders** that were delivered but remain **unbilled**. This represents structural revenue leakage. [V5.0 PRODUCTION]`;
        parsed = {
